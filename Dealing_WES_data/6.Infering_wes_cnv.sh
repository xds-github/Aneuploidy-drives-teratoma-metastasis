...
wig_gz=./mm10.gc200Base.wig.gz
data_folder=./ready_bam/
output_dic=./seq_cnv/
cat ${output_dic}cnv_list.txt | while read id
#sequenza-utils gc_wiggle -w 200 --fasta ./mouse_ensembl_ref.fa -o ./mm10.gc200Base.wig.gz
do
echo "source /home/xudeshu/.bashrc
sequenza-utils bam2seqz -n ./ready_bam5/ES-C.sorted.markdup.realign.BQSR.bam -t ${data_folder}${id}.sorted.markdup.realign.BQSR.bam --fasta ${refer_fa} \
...
echo "qsub -cwd -l vf=5g,h=node5 ${output_dic}${id}.sh" >> ${output_dic}submit.sh
done
